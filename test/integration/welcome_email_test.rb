...
class WelcomeEmailTest < ActionDispatch::IntegrationTest
  ...
  def create_user_as(user, create_user_username, company_id, email, send_mail_bool, user_count_difference, email_validity)
    params = { user: { first_name: "Test", last_name: "User", username: create_user_username, password: "Password1$", company_admin: false, app_admin: false, company_id: company_id, email: email, send_email: send_mail_bool } }
    log_in_if_user(user)
  ...
  test "when a company admin creates a user with a email address and wants a welcome email sent, user should be created and email should send." do
    create_user_as(@company_admin, "new_user", @company_admin_company.id, @email_address, true, 1, true)
  end
  ...
  test "when a company admin creates a user without a email address and wants a welcome email sent, user shouldn't be created and email shouldn't send." do
    create_user_as(@company_admin, "new_user", @company_admin_company.id, nil, true, 0, false)
  ...
  test "when a company admin creates a user with a email address and doesn't want a welcome email sent, user should be created and email shouldn't send." do
    create_user_as(@company_admin, "new_user", @company_admin_company.id, @email_address, false, 1, false)
  end
